...
export const donations = sqliteTable(
  "donations",
  {
    ...
    category: text("category", { enum: ["festival", "idol", "annadaanam", "laddoos"] }).notNull(),
    amount: integer("amount").notNull(),
    paymentMethod: text("payment_method", {
      enum: ["upi", "cash", "bank_transfer", "imps", "neft"],
    }).notNull(),
...
export const culturalProgrammes = sqliteTable(
  "cultural_programmes",
  {
    ...
    performanceType: text("performance_type", { enum: ["solo", "group"] }).notNull().default("solo"),
    category: text("category").notNull(),
    ...
    status: text("status", { enum: ["submitted", "under_review", "clarification_required", "approved", "waitlisted", "scheduled", "completed", "withdrawn"] }).notNull().default("submitted"),
    backgroundMusic: integer("background_music", { mode: "boolean" }).notNull().default(false),
    audioKey: text("audio_key"),
    audioName: text("audio_name"),
    audioType: text("audio_type"),
    stageRequirements: text("stage_requirements").notNull().default(""),
    propsRequirements: text("props_requirements").notNull().default(""),
    setupMinutes: integer("setup_minutes").notNull().default(0),
    source: text("source", { enum: ["resident", "volunteer"] }).notNull().default("volunteer"),
    notes: text("notes").notNull().default(""),
    createdBy: text("created_by").notNull(),
    createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
    updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
