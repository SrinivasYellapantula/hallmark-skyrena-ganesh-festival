  ...
  return <section className="wrap users-shell">
    ...
    <div className="admin-card">
      <form className="user-form" onSubmit={submit}>
        ...
        <label>Role<select name="role" value={role} onChange={(event) => setRole(event.target.value)}><option value="block">Block Coordinator</option><option value="cultural">Cultural Committee</option>{portalOwner&&<option value="admin">Admin</option>}</select></label>
      ...
      <div className="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Block</th><th>Status</th><th>Action</th></tr></thead><tbody>
        {users.map((user) => {const protectedAccount=user.id==="initial-admin"||(!portalOwner&&user.role==="admin");return <tr key={user.id}><td><strong>{user.displayName}</strong><small>{user.username}</small></td><td>{user.id==="initial-admin"?"Portal Admin":user.role === "block" ? "Block Coordinator" : user.role === "cultural" ? "Cultural Committee" : "Admin"}</td><td>{user.blockNo ?? (user.role === "block" ? "—" : "All")}</td><td>{user.active ? "Active" : "Disabled"}</td><td>{protectedAccount?<span className="done">Protected</span>:<button className="button quiet" onClick={() => void toggle(user)}>{user.active ? "Disable" : "Reactivate"}</button>}</td></tr>;})}
      </tbody></table></div>
    </div>
  </section>;
}
