...
type Summary = {
  totals: { festival: number; idol: number; laddoos: number; annadaanam: number; households: number; adults: number; children: number; expenses: number };
...
const empty: Summary = { totals: { festival: 0, idol: 0, laddoos: 0, annadaanam: 0, households: 0, adults: 0, children: 0, expenses: 0 }, blocks: [], donors: [], expenses: [] };
...
export function TransparencyDashboard() {
  ...
  const collected = Number(data.totals.festival) + Number(data.totals.idol) + Number(data.totals.laddoos) + Number(data.totals.annadaanam);
  ...
  return (
    <>
      ...
      <section className={`wrap metrics-grid ${loading ? "is-loading" : ""}`} aria-busy={loading}>
        ...
        <article className="metric"><span>Laddoo fund</span><strong>{currency(Number(data.totals.laddoos))}</strong><small>Dedicated laddoo contributions</small></article>
