...
export function PrasadamSevaForm(){
  ...
  return <>
    ...
    <section className="wrap prasadam-form-shell"><form className="prasadam-seva-form" onSubmit={submit}>
      ...
      <fieldset><legend><span>1</span>Household Details</legend><div className="field-grid">
        <label>Block <b>*</b><select required value={form.blockNo} onChange={(event)=>update("blockNo",event.target.value)}><option value="" disabled>Select block</option>{BLOCKS.map((block)=><option key={block} value={block}>Block {block}</option>)}</select></label>
        <label>Flat Number <b>*</b><input required maxLength={10} value={form.flatNo} onChange={(event)=>update("flatNo",event.target.value.replace(/[\s-]/g,"").toUpperCase())} placeholder="e.g. G01, 505 or 1505"/><small>Do not include the block letter.</small></label>
        <label>Resident Name <b>*</b><input required maxLength={100} value={form.residentName} onChange={(event)=>update("residentName",event.target.value)}/></label>
        <label>Mobile Number <b>*</b><span className="phone-field"><i>+91</i><input required type="tel" inputMode="numeric" pattern="[0-9]{10}" minLength={10} maxLength={10} value={form.phone} onChange={(event)=>update("phone",event.target.value.replace(/\D/g,"").slice(0,10))} placeholder="10-digit number"/></span></label>
      </div></fieldset>
      <fieldset><legend><span>2</span>Offering Details</legend><div className="field-grid">
        <label>Offering Date <b>*</b><select required disabled={!form.blockNo} value={form.offeringDate} onChange={(event)=>update("offeringDate",event.target.value)}><option value="" disabled>{form.blockNo?"Select an eligible date":"Choose your block first"}</option>{dates.map((day)=><option key={day.date} value={day.date}>Day {day.day} · {prasadamDateLabel(day.date)} · {day.label}</option>)}</select>{form.blockNo&&<small>Only Block {form.blockNo}’s assigned day and the open-offering day are available.</small>}</label>
        <label>Prasadam You Wish to Offer <b>*</b><input required maxLength={160} value={form.prasadamName} onChange={(event)=>update("prasadamName",event.target.value)} placeholder="e.g. Pulihora, sweet pongal, laddoo"/></label>
        <label>Number of Portions <b>*</b><input required type="number" inputMode="numeric" min="1" max="2000" value={form.portions} onChange={(event)=>update("portions",event.target.value)} placeholder="People who can be served"/><small>Please enter a practical estimate to help the team plan distribution.</small></label>
        <label>Preparation or Handover Notes <span className="optional">optional</span><textarea rows={3} maxLength={500} value={form.notes} onChange={(event)=>update("notes",event.target.value)} placeholder="Any timing, serving or coordination note for the pooja team"/></label>
      </div></fieldset>
      <aside className="prasadam-guidance"><strong>Please note</strong><ul><li>Prasadam seva is completely voluntary.</li><li>The pooja team may contact you to coordinate quantity, timing and handover.</li><li>Submit a separate entry for every prasadam or date you wish to offer.</li><li>Daily prasadam will be served in the evening after pooja, subject to availability.</li></ul></aside>
