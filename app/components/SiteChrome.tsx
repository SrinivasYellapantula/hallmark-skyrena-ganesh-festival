    ...
    {user && <>
      ...
      <nav id="primary-navigation" className={`site-nav ${navOpen ? "open" : ""}`} aria-label="Primary navigation">
        <div className="nav-destinations">
          ...
          <Link onClick={() => setNavOpen(false)} href="/prasadam-seva/manage">Prasadam Seva</Link>
          {user.role !== "cultural" && <Link onClick={() => setNavOpen(false)} href="/pooja">Pooja Registrations</Link>}
          {(user.role === "admin" || user.role === "cultural") && <Link onClick={() => setNavOpen(false)} href="/cultural">Cultural Programme</Link>}
          {user.role === "admin" && <details ref={adminMenuRef} className="admin-menu"><summary><span>Administration</span><svg className="admin-chevron" aria-hidden="true" viewBox="0 0 12 12"><path d="m2.25 4.25 3.75 3.5 3.75-3.5"/></svg></summary><div><Link onClick={() => setNavOpen(false)} href="/admin"><strong>Collection Verification</strong><small>Collections and payment proofs</small></Link><Link onClick={() => setNavOpen(false)} href="/expenses"><strong>Expenses</strong><small>Fund position, register and receipts</small></Link><Link onClick={() => setNavOpen(false)} href="/meetings"><strong>Meeting Minutes</strong><small>Decisions and action items</small></Link>{user.portalOwner&&<Link onClick={() => setNavOpen(false)} href="/admin#recycle-bin"><strong>Recycle Bin</strong><small>Restore removed records</small></Link>}<Link onClick={() => setNavOpen(false)} href="/admin/users"><strong>User Access</strong><small>Roles and coordinators</small></Link></div></details>}
        </div>
