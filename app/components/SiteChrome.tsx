    ...
    {user && <>
      ...
      <nav id="primary-navigation" className={`site-nav ${navOpen ? "open" : ""}`} aria-label="Primary navigation">
        <div className="nav-destinations">
          {user.role !== "cultural" && <><Link onClick={() => setNavOpen(false)} href="/contribute">New Donation</Link><Link onClick={() => setNavOpen(false)} href="/donations">Donations</Link><Link onClick={() => setNavOpen(false)} href="/pending">Pending Flats</Link></>}
