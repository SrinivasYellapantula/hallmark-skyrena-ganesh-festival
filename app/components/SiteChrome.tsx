    ...
    {user && <>
      ...
      <nav id="primary-navigation" className={`site-nav ${navOpen ? "open" : ""}`} aria-label="Primary navigation">
        <div className="nav-destinations">
          {user.role !== "cultural" && <><Link onClick={() => setNavOpen(false)} href="/contribute">New Donation</Link><Link onClick={() => setNavOpen(false)} href="/donations">Donations</Link><Link onClick={() => setNavOpen(false)} href="/flat-status">Flat Status</Link><Link onClick={() => setNavOpen(false)} href="/pending">Pending Flats</Link></>}
          {(user.role === "admin" || user.role === "cultural") && <Link onClick={() => setNavOpen(false)} href="/cultural">Cultural Programme</Link>}
          {user.role === "admin" && <details className="admin-menu"><summary><span>Administration</span><svg className="admin-chevron" aria-hidden="true" viewBox="0 0 12 12"><path d="m2.25 4.25 3.75 3.5 3.75-3.5"/></svg></summary><div><Link onClick={() => setNavOpen(false)} href="/admin"><strong>Festival Accounts</strong><small>Collections and verification</small></Link><Link onClick={() => setNavOpen(false)} href="/admin#expenses"><strong>Expenses</strong><small>Expense register and receipts</small></Link><Link onClick={() => setNavOpen(false)} href="/meetings"><strong>Meeting Minutes</strong><small>Decisions and action items</small></Link><Link onClick={() => setNavOpen(false)} href="/admin/users"><strong>User Access</strong><small>Roles and coordinators</small></Link></div></details>}
        </div>
