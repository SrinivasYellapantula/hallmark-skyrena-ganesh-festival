...
type FlatInput = { flatNo?: unknown; residentName?: unknown };
...
export async function POST(request: Request) {
  ...
  const flats = body.flats.map((raw) => {
    ...
    return { flatNo, residentName: cleanText(item.residentName, 100) };
  ...
  const statements = flats.map((flat) => d1.prepare(
    `INSERT INTO flats(id,event_id,block_no,flat_no,resident_name,occupied,visit_status,updated_by)
     VALUES(?,?,?,?,?,1,'pending',?)
  ...
  ).bind(crypto.randomUUID(),EVENT_ID,blockNo,flat.flatNo,flat.residentName,auth.user.username));
