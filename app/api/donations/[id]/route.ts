...
async function scopedRegistration(id:string,user:{role:string;blockNo:string|null}) {
  const row=await getD1().prepare(`SELECT id,block_no blockNo,flat_no flatNo,donor_type donorType,status FROM registrations WHERE id=? AND event_id=?`).bind(id,EVENT_ID).first<{id:string;blockNo:string;flatNo:string;donorType:string;status:string}>();
